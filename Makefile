...
provider-template:
	@if [ -z "$(provider)" ]; then echo "Usage: make provider-template provider=<provider_id>"; exit 2; fi
	PYTHONPATH=src $(PY) scripts/provider_authoring.py template --provider-id "$(provider)"

provider-scaffold:
	@if [ -z "$(provider)" ]; then echo "Usage: make provider-scaffold provider=<provider_id>"; exit 2; fi
	PYTHONPATH=src $(PY) scripts/provider_authoring.py scaffold --provider-id "$(provider)"

provider-manifest-update:
	@if [ -z "$(provider)" ]; then echo "Usage: make provider-manifest-update provider=<provider_id>"; exit 2; fi
	PYTHONPATH=src $(PY) scripts/provider_authoring.py update-snapshot-manifest --provider "$(provider)"

provider-validate:
	@if [ -z "$(provider)" ]; then echo "Usage: make provider-validate provider=<provider_id>"; exit 2; fi
	PYTHONPATH=src $(PY) scripts/provider_authoring.py validate-provider --provider "$(provider)"

provider-enable:
	@if [ -z "$(provider)" ]; then echo "Usage: make provider-enable provider=<provider_id> WHY=\"<reason>\" [I_MEAN_IT=1]"; exit 2; fi
	@if [ -z "$(WHY)" ]; then echo "Usage: make provider-enable provider=<provider_id> WHY=\"<reason>\" [I_MEAN_IT=1]"; exit 2; fi
	@flags=""; \
